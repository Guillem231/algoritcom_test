...
export function CameraController({ avatarRef, isManualMode, setIsManualMode }) {
  ...
  useFrame(() => {
    ...
    const avatarPosition = new THREE.Vector3(translation.x, translation.y + 1.5, translation.z);
    ...
    camera.position.set(avatarPosition.x + x, avatarPosition.y + y, avatarPosition.z + z);
    ...
    const handleRightClick = e => {
      if (e.button === 2) {
        ...
        if (!isManualMode) {
          ...
          phiRef.current = Math.acos(
            Math.min(Math.max(offset.y / orbitDistance.current, -0.99), 0.99)
          );
        }
