...
import { MODEL_PATH, ANIMATION_PATHS, ANIMATION_STATES } from '@/modules/avatar/config/animationConfig';
...
export default function FBXAvatar({ 
  ...
  const { model, mixer } = useFBXModel(modelPath, group, handleModelLoaded, handleLoadError, loadAttempts);
  
  
  const { playAnimation } = useAnimationMixer(
    mixer, 
    animationsMap, 
    currentAnimation
  );
